...
heap_t *create_heap(uint32_t start, uint32_t end, uint32_t max, uint8_t supervisor, uint8_t readonly) {
    heap_t *heap = (heap_t*)kmalloc_int(sizeof(heap_t), 0, 0); // Kmalloc_int used for bootstrapping
...
void expand(uint32_t new_size, heap_t *heap) {
    ...
    free((void*)((uint32_t)hole_header + sizeof(header_t)), heap);
}
...
void free(void *p, heap_t *heap) {
    ...
    // Merge right
    header_t *next_header = (header_t *) ((uint32_t)footer + sizeof(footer_t));
    if ((uint32_t)next_header < heap->end_address && next_header->magic == HEAP_MAGIC && next_header->is_hole) {
    ...
    if ((uint32_t)prev_footer > heap->start_address && prev_footer->magic == HEAP_MAGIC && prev_footer->header->is_hole) {
